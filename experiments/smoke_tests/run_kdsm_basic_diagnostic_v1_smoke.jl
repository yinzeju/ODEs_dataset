## Purpose and smoke release scope
...
## Generate bounded smoke objects

function run_kdsm_basic_diagnostic_v1_smoke()
    return kbasic_run_release_generation(PROJECT_ROOT; difficulty = "smoke")
end

if abspath(PROGRAM_FILE) == abspath(@__FILE__)
    result = run_kdsm_basic_diagnostic_v1_smoke()
    kbasic_print_release_summary(result)
    result["release_index"]["all_passed"] || error("kdsm_basic_diagnostic_v1 smoke generation failed diagnostics")
end
