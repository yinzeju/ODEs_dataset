...
function kdsm_split_counts(split_config::AbstractDict, difficulty::AbstractString)
    key = difficulty == "bounded" ? "bounded_counts" : "default_counts"
...
function kdsm_build_trajectory_split_roles(
    R::Integer,
    split_config::AbstractDict;
    difficulty::AbstractString = "default",
)
    counts = kdsm_split_counts(split_config, difficulty)
    ...
    kdsm_validate_split_roles(roles, counts)
    return roles, counts
end

## Split leakage checks

function kdsm_validate_split_roles(
    roles::AbstractVector{<:AbstractString},
    counts::AbstractDict,
)
    all(role -> role in ("train", "val", "test"), roles) ||
        throw(ArgumentError("split_roles contains invalid role"))
