...
This task generated the dedicated Lorenz96 complete-state dataset requested by `docs/notes/mathematical explanation/lorenz96_nx40_complete_state_v1.md`. The dataset fixes `N_x=40`, `F0=8`, RK4 internal step `dt_internal=0.005`, save stride `q_save=10`, learning interval `tau=0.05`, burn-in time `100.0`, and `M_traj=2048` one-step pairs per trajectory. The current 2026-07-10 formal release contains 480 independent burn-in trajectories split by trajectory into 288 train, 96 validation, and 96 test trajectories. It stores raw physical-coordinate trajectories only and does not provide an in-object standardization or normalization resource.

The generated learning object is the full physical state with `z_m = y_m = x_m`. No observation noise, partial observation, control input, delay embedding, spatial patching, data augmentation, model training, kernel selection, spectrum estimation, standardization, or normalization was added.
...
- `src/data/l96_nx40_complete_state_v1_generation.jl`: reusable implementation for seed spawning, Lorenz96 RK4 integration, split tensor writing, diagnostics, report tables, and report generation.
...
- `src/data/l96_nx40_complete_state_v1_generation.jl` implements a preallocated Float64 RK4 kernel for Lorenz96 with periodic indexing, deterministic trajectory seed spawning from master seed `20260624`, raw split tensor writing, and formal acceptance checks.
...
The experiment entry point includes `src/data/l96_nx40_complete_state_v1_generation.jl` and calls `generate_l96_nx40_complete_state_v1(PROJECT_ROOT)`. The generator builds independent trajectory seeds, integrates each pre-burn-in initial condition for 20,000 internal RK4 steps, records 2049 snapshots at stride 10 internal steps, partitions the raw trajectory tensor into split-level JLD2 files, runs acceptance diagnostics, removes any stale `normalization.json`, and writes JSON/CSV/Markdown outputs.
...
| In-object standardization resource | none |
