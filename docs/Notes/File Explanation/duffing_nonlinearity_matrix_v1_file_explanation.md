...
This task generated the formal `duffing_nonlinearity_matrix_v1` dataset from `docs/notes/mathematical explanation/duffing_nonlinearity_matrix_v1_math.md`.

The release contains 42 objects: 2 linear health-check objects and 40 Duffing cells over `beta in {0, 0.05, 0.2, 0.5, 1, 2, 5, 10}` and `Q in {0.25, 0.5, 1.0, 1.5, 2.0}`. Every object uses shape `(512, 1025, 2)`, trajectory-level split counts `384/64/64`, clean full-state observations, and target tensors equal to the physical state.
...
- `src/generators/duffing_nonlinearity_matrix_v1_generator.jl` loads configs, builds the 42-object plan, creates shared initial-condition libraries by `Q`, builds split/window metadata, saves raw and processed JLD2 tensors, writes object/release manifests, writes report tables, and validates the release.
...
3. The generation plan creates two linear base specs and forty Duffing cell specs.
...
- `object_count = 42`
- `duffing_cell_count = 40`
- all tensors have shape `(512, 1025, 2)`
- split counts are `384/64/64`
- `chi_nl` range is `[0, 40]`
- maximum state absolute value is `8.81383143591368`
- maximum energy violation count is `0`
- maximum linear recurrence residual is `6.66133814775094e-16`
